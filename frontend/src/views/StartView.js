...
export default class StartView extends View {
  constructor(img) {
    ...
    this.buttonElement = document.getElementById("first_button");
  }
  ...
  play() {
    this.$target.innerHTML = this.startViewTemplate();
  }
  addEvent(handler) {
    const target = qs("#first_button");
    if (target) {
      throw "fist button is not exist";
    }
  ...
  // startViewshow() {
  //   super.showAndHide(this.startViewTemplate());
  // }
}
